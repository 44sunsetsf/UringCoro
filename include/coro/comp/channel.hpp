...
class channel_base {
public:
  ~channel_base() noexcept {
    assert(part_closed() && "detected channel destruct with no_close state");
  }

  auto close() noexcept -> void {
    std::atomic_ref<uint8_t>(m_close_state)
        .store(part_close, std::memory_order_release);
...
protected:
  inline auto complete_closed_atomic() noexcept -> bool {
    return std::atomic_ref<uint8_t>(m_close_state)
               .load(std::memory_order_acquire) <= complete_close;
  }

  inline auto part_closed_atomic() noexcept -> bool {
    return std::atomic_ref<uint8_t>(m_close_state)
               .load(std::memory_order_acquire) <= part_close;
  }

  inline auto part_closed() noexcept -> bool {
    return m_close_state <= part_close;
  }
...
protected:
  ...
  alignas(std::atomic_ref<uint8_t>::required_alignment) uint8_t m_close_state{
      no_close};
};
}; // namespace detail
...
public:
  ...
  auto send(value_type &&value) noexcept -> task<bool> {
    ...
    co_await m_producer_cv.wait(
        m_mtx, [this]() -> bool { return !full() || part_closed(); });
  ...
  auto recv() noexcept -> task<data_type> {
    ...
    auto lock = co_await m_mtx.lock_guard();
    co_await m_consumer_cv.wait(
        m_mtx, [this]() -> bool { return !empty() || part_closed(); });
